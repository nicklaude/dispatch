...
class TestRegistryBasicOps:
    ...
    def test_remove(self, registry):
        registry.register(chat_id="test:+11111111111", session_name="user1")
        registry.remove("test:+11111111111")
        assert registry.get("test:+11111111111") is None
    ...
    def test_get_by_session_name(self, registry):
        registry.register(chat_id="test:+11111111111", session_name="user-one-test")
        result = registry.get_by_session_name("user-one-test")
        assert result is not None
        assert result["chat_id"] == "test:+11111111111"
...
class TestRegistryPersistence:
    ...
    def test_preserves_created_at_on_update(self, registry):
        registry.register(chat_id="test:+11111111111", session_name="u1")
        created_at = registry.get("test:+11111111111")["created_at"]

        time.sleep(0.01)
        registry.register(chat_id="test:+11111111111", session_name="u1", extra="data")
        assert registry.get("test:+11111111111")["created_at"] == created_at
...
class TestRegistrySessionTracking:
    ...
    def test_update_session_id(self, registry):
        registry.register(chat_id="test:+11111111111", session_name="u1")
        registry.update_session_id("test:+11111111111", "sdk-session-abc123")
        data = registry.get("test:+11111111111")
    ...
    def test_update_last_message_time(self, registry):
        registry.register(chat_id="test:+11111111111", session_name="u1")
        registry.update_last_message_time("test:+11111111111")
        data = registry.get("test:+11111111111")
